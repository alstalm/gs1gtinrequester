    ...
    for infotype_record in range(len(common_part['InfoTypeRecords']['record'])):
        try: # если в infotype_record есть AttributeValues и это или список или словарь
            if  isinstance(common_part['InfoTypeRecords']['record'][infotype_record]['AttributeValues']['value'], list): # если в InfoTypeRecords/record/AttributeValues несколько value
                ...
                for value_number in range(len(common_part['InfoTypeRecords']['record'][infotype_record]['AttributeValues']['value'])):
                    try: # проверяем если есть вложенный мультиатрибут
                            ...
                            if isinstance(common_part['InfoTypeRecords']['record'][infotype_record]['AttributeValues']['value'][value_number]['ns0:MultValue']['@extAttrId'], str): # если multivalue одно
                                ...
                                if web_attr_id not in web_attr_list:
                                    ...
                                    try:
                                        ...
                                        df.loc[global_record, web_attr_id] = web_attr_value
                                        print('wap53: текущее значение df= \n {}\n'.format(df))

                            else: # если multivalue  НЕ одно
                                print('\nwap56: в infotype_record N={} в /AttributeValues в записи  value N={} ВОЗМОЖНО несколько записей MultValue. проверим.'.format(infotype_record, value_number))
                                ...
                                if web_attr_id not in web_attr_list:
                                    ...
                                    for MultValue_N in range(len(common_part['InfoTypeRecords']['record'][infotype_record]['AttributeValues']['value'][value_number]['ns0:MultValue'][MultValue_N])):
                                        try:
                                            ...
                                            multiattrlist.append(web_attr_value)
                        ...
                        print('wap72: в infotype_record={} value_number={} парсим web_attr_id={}'.format(infotype_record, value_number, web_attr_id))

                        if web_attr_id not in web_attr_list:
                            print('wap75: НО web_attr_id = {} НЕ в списке заданных атрибутов :(\n'.format(web_attr_id))
                            continue
                        else:
                            try:
                                ...
                                df.loc[global_record, web_attr_id] = web_attr_value
                                print('wap88: для value={} текущее значение web_attr_value={}'.format(value_number, web_attr_value))
                                print('wap89: web_attr_id = {} находится в списке заданных атрибутов'.format(web_attr_id))
                                print('wap90: текущее значение df= \n {}\n'.format(df))
                            except:
                        ...
                        print('wap94: в infotype_record N={} в /AttributeValues в записи  value N={} что-то НЕОЖИДАННОЕ'.format(infotype_record, value_number))
            ...
            if isinstance(common_part['InfoTypeRecords']['record'][infotype_record]['AttributeValues']['value'], dict): # если только одна запись value в AttributeValues
                print('wap97: в infotype_record N={} в AttributeValues/value только одна запись value.'.format(infotype_record))
                try:  # проверяем если мультиатрибут один
                    if isinstance(       common_part['InfoTypeRecords']['record'][infotype_record]['AttributeValues']['value']['ns0:MultValue']['@extAttrId'], str) \
                        ...
                        print('wap111: в записи infotype_record N={} в /AttributeValues в единственной записи value есть одна или несколько MultValue'.format(infotype_record))
                        if isinstance(       common_part['InfoTypeRecords']['record'][infotype_record]['AttributeValues']['value']['ns0:MultValue']['@extAttrId'], str):
                            ...
                            try:
                                ...
                                if web_attr_id not in web_attr_list:
                                    print('wap131: web_attr_id {} not in web_attr_list web_attr_list'.format(web_attr_id))
                                else:
                                    df.loc[global_record, web_attr_id] = web_attr_value
                                    print('wap134: текущее значение df= \n {}\n'.format(df))


                        else:
                            print('\nwap138: в infotype_record N={} в /AttributeValues в единственной записи value НЕСКОЛЬКО записей MultValue'.format(infotype_record))
                            multiattrlist = []
                            for MultValue_N in range(len(common_part['InfoTypeRecords']['record'][infotype_record]['AttributeValues']['value']['ns0:MultValue'][MultValue_N])):
                                ...
                                try:
                                    ...
                                    if web_attr_id not in web_attr_list:
                                        ...
                                        print('wap159: текущее значение df= \n {}\n'.format(df))
                            df.loc[global_record, web_attr_id] = multiattrlist
                    ...
                    try:
                        ...
                        if web_attr_id not in web_attr_list:
                            print('wap178: web_attr_id {} not in web_attr_list web_attr_list'.format(web_attr_id))
                        else:
                            df.loc[global_record, web_attr_id] = web_attr_value

                            print('wap182: текущее значение df= \n {}\n'.format(df))

                except :  #
                    print('wap181: в infotype_record N={} в /AttributeValues в записи  value N={} что-то НЕОЖИДАННОЕ'.format(infotype_record, value_number))
        except TypeError: #  нет  пути InfoTypeRecords/record/AttributeValues нет или там только одно value
            print('wap187: в infotype_record {} НЕ содержится AttributeValues'.format(infotype_record))
