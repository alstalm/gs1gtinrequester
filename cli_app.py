...
def parse_args():
    ...
    subparser_from_file.add_argument("-n", "--no_valueMap", help="отключает запрос в БД для получения valueMap", action='store_true', )
    ...
    subparser_from_clipboard.add_argument('-g', '--gtins', nargs='+', help='список gtin через пробел', type=int, required=False)
    subparser_from_clipboard.add_argument('-a', '--attributes', nargs='+', help='список GS1Attr через пробел', required=False)

    subparser_from_clipboard.add_argument("-n", "--no_valueMap", help="отключает запрос в БД для получения valueMap", action='store_true', )
